    ...
    def model_fn(x, t_continuous):
        ...
        if guidance_type == "uncond":
            ...
            if guidance_scale == 1. or unconditional_condition is None:
                ...
                rescale_cfg = guidance_scale
                if model_kwargs.get('cfg_schedule', False) == 'cosine':
                    rescale_cfg = (guidance_scale) * (torch.cos(math.pi * (1 - t_continuous)) + 1) / 2
                elif model_kwargs.get('cfg_schedule', False) == 'linear':
                    rescale_cfg = guidance_scale * t_continuous
                # print(f'CFG({t_continuous}) = {rescale_cfg}')
                x_in = torch.cat([x] * 2)
                t_in = torch.cat([t_continuous] * 2)
                c_in = torch.cat([condition, unconditional_condition], dim=2)
                noise, noise_uncond = noise_pred_fn(x_in, t_in, cond=c_in).chunk(2)
                return noise_uncond + rescale_cfg * (noise - noise_uncond)
