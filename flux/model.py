...
def load_flow_model(name: str, ckpt_path: str, device: Union[str, torch.device] = "cuda"):
    # Loading Flux
    print("Init model")
    with torch.device("meta" if ckpt_path is not None else device):
        model = Flux(configs[name].params).to(torch.float8_e4m3fn)
